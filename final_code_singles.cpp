...
int dijk_orig(const graph &G,const edge_array<int>& cost, node s, node t,node_array<int>& number) {
  node_pq<double> PQ(G);
  node v; edge e;
...
float T = used_time();
  vertexCnt_orig= dijk_orig(G, d, st, tg,number);
